...
export async function createPrescription(req, res) {
    try {
        ...
        // AUTO-GENERATE APPOINTMENT FOR WALK-IN/DIRECT PRESCRIPTIONS
        // This ensures the doctor gets financial credit and statistical tracking
        // for patients they started writing for without creating an appointment first!
        let resolvedAppointmentId = data.appointmentId;
        if (!resolvedAppointmentId && data.chamberId) {
            const Appointment = (await import('../Appointment/Appointments.model.js')).default;
            
            // Calculate today's next serial for the chamber
            ...
            const nextSerial = lastAppt ? lastAppt.serial + 1 : 1;
            const padVal = String(nextSerial).padStart(4, "0");
            const newAppointmentGenId = `AP${Date.now().toString().slice(-4)}${padVal}`;

            const autoGeneratedAppt = await Appointment.create({
                appointmentId: newAppointmentGenId,
                ...
                isPrescription: "Yes",
                branch: data.branch,
                paymentStatus: "Collect" // They wrote rx directly, assume payment collected
            });

            resolvedAppointmentId = autoGeneratedAppt._id;
        }
...
// Get Prescription Stats (Calculate patient types)
export async function getPrescriptionStats(req, res) {
    const branch = req.params.branch;
    try {
        ...
        stats.forEach(stat => {
            if (stat._id === 'New Patient') newCount = stat.count;
            else if (stat._id === 'Old Patient') oldCount = stat.count;
            else if (stat._id === 'Report') reportCount = stat.count;
            else unassignedCount += stat.count;
        });

        res.status(200).json({
            success: true,
            data: { newCount, oldCount, reportCount, unassignedCount }
        });
